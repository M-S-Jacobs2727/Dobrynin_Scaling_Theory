...
def surface_generator_Bth(num_batches: int, batch_size: int,
    ...
    def generate_surfaces(Bg, Bth, Pe, a):
        ...
        # Calculate parameters used in each Ne conjecture calculation
        ...
        # Calculate lam_g for each Ne conjecture
        ...
        # Calculate lam for each Ne conjecture
        ...
        # Assign values of lam_g depending on
        # Whether Bg value was included in generating eta_sp surface (a_tile==0)
        ...
        for i in range(0,batch_size):
            ...
            arr_choose = arr_all[torch.sum(eta_sp[i] > 0, dim=1) > num_eta_sp_threshold]
            ...
            if return_nw:
                ...
                Nw_min[i] = np.min(Nw[0, :, 0][trim_nw_arr].detach().cpu().numpy())
                Nw_max[i] = np.max(Nw[0, :, 0][trim_nw_arr].detach().cpu().numpy())
                Num_Nw[i] = len(Nw[0, :, 0][trim_nw_arr].detach().cpu().numpy())
            ...
            trim_phi_arr = torch.randint(tensor_phi_arr.min(), tensor_phi_arr.max(), (5, 13)).unique().to(device)
    ...
    for _ in range(num_batches):
        ...
        if return_nw:
            eta_sp, Nw_min, Nw_max, Num_Nw = generate_surfaces(Bg, Bth, Pe, a)
        else:
