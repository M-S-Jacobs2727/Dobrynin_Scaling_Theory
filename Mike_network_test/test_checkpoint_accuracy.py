...
def test_accuracy(
    model: torch.nn.Module, loss_fn: torch.nn.Module, device: torch.device,
    num_samples: int, batch_size: int, res: Tuple[int]
) -> Tuple[torch.Tensor]:
...
def main() -> None:
    ...
    plt.plot(
        bg_true.flatten(), bg_pred.flatten(),
        marker='o', mec='r', mfc='None', mew=1, ls='None'
    )

    plt.figure('Bth')
    plt.plot([0, 1], [0, 1], 'k-', lw=3)
    plt.plot(
        bth_true.flatten(), bth_pred.flatten(),
        marker='o', mec='b', mfc='None', mew=1, ls='None'
    )

    plt.figure('Pe')
    plt.plot([0, 1], [0, 1], 'k-', lw=3)
    plt.plot(
        pe_true.flatten(), pe_pred.flatten(),
        marker='o', mec='g', mfc='None', mew=1, ls='None'
    )
    plt.show()

    bg_true, bth_true, pe_true = bg_true.flatten(), bth_true.flatten(), pe_true.flatten()
    bg_pred, bth_pred, pe_pred = bg_pred.flatten(), bth_pred.flatten(), pe_pred.flatten()
    ...
    numpy.savetxt(
        ...
        torch.stack(
            (bg_true, bg_pred, bth_true, bth_pred, pe_true, pe_pred),
            dim=1
        ).numpy(),
