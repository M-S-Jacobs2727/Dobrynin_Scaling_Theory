...
def surface_generator_corr_B(
    ...
    for _ in range(num_batches):
        ...
        Bg, Pe = unnormalize_params(*(y_temp.T))
        Bth = get_Bth(Bg)
        eta_sp = generate_surfaces(Bg, Bth, Pe)
        X = normalize_visc(eta_sp).to(torch.float)
        _, y1, _ = normalize_params(torch.zeros(1), Bth, torch.zeros(1))
        y = torch.stack((y_temp[:, 0], y1, y_temp[:, 1]), dim=1).to(torch.float)
        yield X, y
...
def main() -> None:
    ...
    for b, (X, _) in enumerate(
        surface_generator(
            num_batches, batch_size, torch.device("cuda"), Resolution(64, 64)
        )
    ):
        i = b * batch_size
        j = i + b  # PEP8 vs flake8...
        frac[i:j] = torch.sum(X == 0, dim=(1, 2))
