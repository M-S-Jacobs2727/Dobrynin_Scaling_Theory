...
def load_exp_data(df, device, resolution, batch_size):
    ...
    for i in np.unique(df2['group']):
        ...
        #X[counter] = torch.from_numpy(data_save).to(device)
...
def main():
    ...
    checkpoint = torch.load("../model_best_accuracy_chkpt_batch_64_renorm.pt")
    ...
    plot_eval.plot_data(y_unnorm, pred_unnorm)
