...
def load_exp_data(df, device, resolution, batch_size):
    ...
    X = unnorm_eta_sp(X)
    X_131 = unnorm_eta_sp_131(X_131)
    X_2 = unnorm_eta_sp_2(X_2)
    ...
    return X_stack, y, df_create, X, X_131, X_2
...
def main():
    ...
    X, y, table, eta_sp, eta_sp_131, eta_sp_2 = load_exp_data(df, device, resolution, batch_size)
    ...
    with torch.no_grad():
        ...
        Pe = ryan.recalc_pe_prediction(device, Bg, Bth, eta_sp, resolution, batch_size, pe_true).to(device)
    ...
    plot_eval.record_fits(df, pred_unnorm_data, eta_sp, eta_sp_131, eta_sp_2, batch_size, device)
