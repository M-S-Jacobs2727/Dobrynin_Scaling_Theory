...
def custom_MSE_loss(y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
    ...
    loss = torch.sum(torch.mean((y_true[:, 0] - y_pred[:, 0]) ** 2) + torch.mean((y_true[mask][:, 1] - y_pred[mask][:, 1]) ** 2) + torch.mean(y_true[:, 2] - y_pred[:, 2]) ** 2)
...
def custom_MAE_loss(y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
    ...
    loss = torch.sum(torch.mean(torch.abs(y_true[:, 0] - y_pred[:, 0])) + torch.mean(torch.abs(y_true[mask][:, 1] - y_pred[mask][:, 1])) + torch.mean(torch.abs(y_true[:, 2] - y_pred[:, 2])))
