...
def test_model(model, device, resolution, X_131, table_vals):
    ...
    bg_error = test_gen.get_error_bg(table_vals)
...
def main():
    ...
    print(f'Epoch\ttrain_loss\tBg Train Error\ttest_loss\tBg Test Error\tBg Exp Data Error\ttime', flush=True)

    best_ave_accuracy = 100
    best_validation_loss = 100
    optimizer = torch.optim.Adam(model.parameters(),
    lr=0.00017157879282447955,
    betas=(0.8178017179262261,0.9886582276361054),
    weight_decay=0,
    eps=2.307354314546036e-08
    )
    ...
    for m in range(epochs):
        ...
        test_exp_data_error = test_model(model, device, resolution, X_131, table_vals)
        ...
        print(f'{m}\t{train_loss:>8f}\t{bg_train_error:.4f}\t{test_loss:>8f}\t{bg_valid_error:.4f}\t{test_exp_data_error:.4f}\t{elapsed:.2f}', flush=True)
