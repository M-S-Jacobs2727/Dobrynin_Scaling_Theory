...
def main():
    ...
    checkpoint = torch.load("/media/sayko/storage1/CNN-Model-Analysis/VGG/Bg_vgg_3_model_best_exp_error.pt",map_location=device)
    ...
    checkpoint = torch.load("/media/sayko/storage1/CNN-Model-Analysis/VGG/Bth_vgg_3_model_best_exp_error.pt",map_location=device)
    model.load_state_dict(checkpoint['model_state_dict'])
    epoch = checkpoint['epoch']
    X_2 = X_2.to(device)
    model.eval()
