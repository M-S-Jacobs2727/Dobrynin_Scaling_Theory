...
EXP_BG_MU = -0.54748
EXP_BG_STDEV = 0.30963
EXP_BTH_MU = -1.05196
EXP_BTH_STDEV = 0.2248
EXP_PE_MU = 1.94802
EXP_PE_STDEV = 0.49004
...
def bth_regularization(Bg: torch.Tensor, Bth: torch.Tensor, device: torch.device):
    """ Regularize Bth
    With our experimental data, min(Bth/Bg) ~ 0.45 and max(Bth/Bg) ~ 0.8.
    Lets try generating data such that Bth correlates with Bg within these parameters.
    """
    upper = 0.8
    lower = 0.45
    mask = (Bth>Bg*upper) | (Bth<Bg*lower)
    Bth_regularize = torch.rand(len(Bth[mask])).to(device)*(Bg[mask]*upper-Bg[mask]*lower)+Bg[mask]*lower
    Bth[mask] = Bth_regularize

    # cap upper and lower bounds
    mask_lower = Bth<Bg*lower
    mask_upper = Bth>Bg*upper
    Bth[mask_lower] = Bg[mask_lower]*lower
    Bth[mask_upper] = Bg[mask_upper]*upper
    Bth[Bth>0.65] = 0.65
    #Bth[Bth<0.2] = 0.2 + 1e-4      # not needed, as (Bg_min * min(Bth/Bg)) = 0.45*0.45 = 0.2025
    return Bth
...
def surface_generator(num_batches: int, batch_size: int,
    ...
    for _ in range(num_batches):
        ...
        #Pe = torch.rand(batch_size, dtype=torch.float).to(device)
        y = torch.column_stack((Bg, Bth, Pe)).to(device)
        #Pe = unnormalize_Pe(y[:,2])
        #y[:,2] = Pe
        ...
        #y = torch.normal(mean= (batch_size, 3), device=device, dtype=torch.float)
        #Bg, Bth, Pe = unnormalize_params(y)
        #Bth = bth_regularization(Bg, Bth, device)       # regularize Bth
        #eta_sp = generate_surfaces(Bg, Bth, Pe)
        #X = normalize_visc(eta_sp).to(torch.float)
        #y[:,1] = normalize_bth(Bth)             # set Bth col for y to new Bth vals
        #yield X, y
