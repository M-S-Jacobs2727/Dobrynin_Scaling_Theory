...
def state_diagram(df_all, df_table, device):
    ...
    df_create.to_csv("vgg-df_state_diagram.csv")
    #print(df_create)
...
def universal_plot(df_all, df_table, device):
    ...
    phi_th_t = Bth_t**3*(Bth_t/Bg_t)**(1/(2*0.588-1))
    Bth6_t = Bth_t**6
    ...
    # forgot "g_" for lam_g_Bth
    lam_g = torch.where(torch.isnan(param_t), lam_g_Bth, torch.where(param_t >= 1, lam_g_KN, lam_g_RC))
    ...
    df2.to_csv("vgg-df_universal_plot.csv")
    #print(df2)
