...
#ETA_SP_131 = Param(torch.tensor(0.1), torch.tensor(1000))
#ETA_SP_2 = Param(torch.tensor(1), torch.tensor(20000))
ETA_SP_131 = Param(torch.tensor(0.1), torch.tensor(1.4e5))
...
def surface_generator(num_batches: int, batch_size: int,
    ...
    def generate_surfaces(Bg, Bth, Pe):
        ...
        # Nw trim
        trim = torch.randint(0, resolution[0], (batch_size,2)).sort(1)[0]
        arr = torch.arange(0, resolution[0])

        for i in range(0,batch_size):

            arr2 = arr[(arr < trim[i][0]) | (arr > trim[i][1])]
            eta_sp[i, arr2] = 0
    ...
    for _ in range(num_batches):
        ...
        X_test = torch.stack((X_131, X_2), dim=1)
        
        yield X_test, y2, y3, eta_sp
