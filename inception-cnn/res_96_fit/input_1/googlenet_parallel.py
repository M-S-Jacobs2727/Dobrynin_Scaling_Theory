...
def main(d, model):
    ...
    torch.distributed.init_process_group(backend='nccl', world_size=2, init_method=None, rank=d)
    ...
    for batch in batch_size:
        ...
        for m in range(epochs):
            ...
            train_y_cat = torch.cat((train_y_list[0], train_y_list[1])).to(0)
            ...
            train_pred_cat = torch.cat((train_pred_list[0], train_pred_list[1])).to(0)
            ...
            valid_y_cat = torch.cat((valid_y_list[0], valid_y_list[1])).to(0)
            ...
            valid_pred_cat = torch.cat((valid_pred_list[0], valid_pred_list[1])).to(0)
            ...
            if d == 0:
                ...
                if (torch.mean(train_error[0] + train_error[1]) + torch.mean(test_error[0] + test_error[1])) / 2.0 <= best_ave_accuracy:
                    ...
                    }, "model_best_accuracy_chkpt_diagnose.pt")
                ...
                if test_loss < best_validation_loss:
                    ...
                    'model_state_dict': model.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict()
                    }, "model_best_accuracy_loss_diagnose.pt")
...
if __name__ == '__main__':
    ...
    assert n_gpus >= 2, f"Requires at least 4 GPUs to run, but got {n_gpus}"
    model = InceptionV1()
