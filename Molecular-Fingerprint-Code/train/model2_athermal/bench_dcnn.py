...
def train( 
    ...
    train_pred = torch.zeros(num_samples, 2).to(device)
    train_y = torch.zeros(num_samples, 2).to(device)
...
def validate( 
    ...
    all_data_pred = torch.zeros(num_samples, 2).to(device)
    all_data_y = torch.zeros(num_samples, 2).to(device)
...
def test(
    ...
    eval_data_pred = torch.zeros(num_samples, 2). to(device)
    eval_data_y = torch.zeros(num_samples, 2).to(device)
...
def main(d, model): 
    ...
    if d==0:
        print(f'Epoch\ttrain_loss\ttrain_err[0]\ttrain_err[1]\ttest_loss\ttest_err[0]\ttest_err[1]\ttime', flush=True)
    ...
    for m in range(epochs):
        ...
        train_y_list = [torch.zeros((train_size,2), dtype=torch.float, device=device) for _ in range(4)]
        ...
        train_pred_list = [torch.zeros((train_size,2), dtype=torch.float, device=device) for _ in range(4)]
        ...
        valid_y_list = [torch.zeros((test_size,2), dtype=torch.float, device=device) for _ in range(4)]
        ...
        valid_pred_list = [torch.zeros((test_size,2), dtype=torch.float, device=device) for _ in range(4)]
        ...
        if d == 0:
            ...
            print(f'{m+1}\t{train_loss:>5f}\t{train_error[0]:.4f}\t{train_error[1]:.4f}\t{test_loss:>5f}\t{test_error[0]:.4f}\t{test_error[1]:.4f}\t{elapsed:.2f}', flush=True)
    ...
    eval_y_list = [torch.zeros((eval_size,2), dtype=torch.float, device=device) for _ in range(4)]
    dist.all_gather(eval_y_list, eval_y)
    eval_y_cat = torch.cat((eval_y_list[0], eval_y_list[1], eval_y_list[2], eval_y_list[3])).to(0)

    eval_pred_list = [torch.zeros((eval_size,2), dtype=torch.float, device=device) for _ in range(4)]
    ...
    if d==0:
        ...
        print(f'Epoch\teval_loss\teval_err[0]\teval_err[1]\teval_err[2]', flush=True)
        print(f'{epoch}\t{eval_loss:>5f}\t{eval_error[0]:.4f}\t{eval_error[1]:.4f}', flush=True)
