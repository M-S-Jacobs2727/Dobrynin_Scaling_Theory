...
class InceptionAux(torch.nn.Module):
    ...
    def __init__(self, in_channels, num_classes):
        ...
        #self.pool = torch.nn.AvgPool2d(kernel_size=5, stride=6)
...
def train( 
    ...
    for b, (X, y2, y3, eta_sp) in enumerate(mike.surface_generator(num_batches, batch_size, device, resolution=resolution)):
        ...
        Bth = pred[:,1]
        #Pe_old = pred[:,2]
        #pred_new = torch.clone(Bg)
        Pe_new = ryan.recalc_pe_prediction(device, Bg, Bth, eta_sp, resolution, batch_size).to(device)
        pred_new = torch.column_stack((Bg, Bth, Pe_new))
        pred_new[:,2] = mike.normalize_Pe(pred_new[:,2].squeeze())
        loss_main = loss_fn(pred, y2)
        loss = loss_main
        loss.backward()
        optimizer.step()
        avg_loss += loss
        avg_error += torch.mean(torch.abs(y3 - pred_new) / y3, 0)

        train_pred[counter:counter+batch_size] = pred_new
        train_y[counter:counter+batch_size] = y3
...
def validate( 
    ...
    with torch.no_grad():
        for b, (X, y2, y3, eta_sp) in enumerate(mike.surface_generator(num_batches, batch_size, device, resolution=resolution)):
            ...
            all_data_y[counter:counter+batch_size] = y3
            counter = counter + batch_size
