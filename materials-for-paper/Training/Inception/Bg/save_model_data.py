...
def save_model_and_data_loss(model, optimizer, epoch, train_y_cat, valid_y_cat, train_pred_cat, valid_pred_cat):
    ...
    train_y_data = train_y_cat.cpu().numpy()
    df_train_y = pd.DataFrame(train_y_data)
    df_train_y.to_csv("train_y_data_loss.csv",index=False)

    valid_y_data = valid_y_cat.cpu().numpy()
    df_valid_y = pd.DataFrame(valid_y_data)
    df_valid_y.to_csv("valid_y_data_loss.csv",index=False)

    train_pred_data = train_pred_cat.cpu().numpy()
    df_train_pred = pd.DataFrame(train_pred_data)
    df_train_pred.to_csv("train_pred_data_loss.csv",index=False)

    valid_pred_data = valid_pred_cat.cpu().numpy()
    df_valid_pred = pd.DataFrame(valid_pred_data)
    df_valid_pred.to_csv("valid_pred_data_loss.csv",index=False)
