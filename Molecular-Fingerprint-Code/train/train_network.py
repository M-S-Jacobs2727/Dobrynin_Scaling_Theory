...
def main(): 
    ...
    for m in range(1):
        ...
        for i in range(20):
            ...
            if (np.sum(train_error.cpu().detach().numpy()<0.05)==2) and (np.sum(test_error.cpu().detach().numpy()<0.05)==2) and (train_error[2].cpu().detach().numpy()<0.05) and (test_error[2].cpu().detach().numpy()<0.05):
                torch.save(model.state_dict(), "model_"+str(m)+"_100_batch_5_percent.pt")
                print("Model saved!")
